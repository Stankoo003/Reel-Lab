...
class ProfileIntegrationTest extends ApiIntegrationTest {
    ...
    /**
     * An avatar reaches the client as an absolute URL wherever a user appears — not only on
     * the profile screen. A comment author with no picture is the one place a face is most
     * useful, and it used to be the one place the URL was missing.
     *
     * <p>The relative path is what the database stores; this asserts the composition, so a
     * response that leaked the raw path would fail here rather than 404 on a device.
     */
    @Test
    void everyShapeThatCarriesAUserCarriesTheirAvatarAsAnAbsoluteUrl() throws Exception {
        String path = uploadAvatar("me.png", "image/png", PNG_HEADER, 1_000);
        okBody(patch("/api/users/{id}", alice.getId())
                .header(HttpHeaders.AUTHORIZATION, bearer(alice))
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"displayName\":\"Alice\",\"avatarPath\":\"" + path + "\"}"));

        VideoEntity video = publishedVideo(alice, "clip");
        comments.save(new CommentEntity(video, alice, null, "hello"));

        JsonNode[] users = {
            okJson(get("/api/users/{id}/profile", alice.getId())),
            okJson(get("/api/users/{id}", alice.getId())),
            okJson(get("/api/videos/{id}", video.getId())).get("owner"),
            okJson(get("/api/videos/{id}/comments", video.getId())).get("items").get(0).get("author"),
            okJson(get("/api/users/search").param("q", "alice")).get(0),
        };

        for (JsonNode user : users) {
            String url = user.get("avatarUrl").asText();
            assertThat(url).as("absolute, and never the stored relative path").startsWith("http");
            assertThat(url).endsWith(path);
        }
    }

    /**
     * The real name is its own field, editable and clearable like the bio.
     *
     * <p>Not derived from the display name, and never backfilled from it: a display name is
     * what someone chose to be called, which is not evidence of what they are called.
     */
    @Test
    void aRealNameCanBeSetChangedAndCleared() throws Exception {
        assertThat(okJson(get("/api/users/{id}/profile", alice.getId())).get("fullName").isNull())
                .as("nobody starts with one").isTrue();

        assertThat(patchProfile("{\"displayName\":\"Alice\",\"fullName\":\"Alice Petrovic\"}")
                .get("fullName").asText()).isEqualTo("Alice Petrovic");

        // Changed.
        assertThat(patchProfile("{\"displayName\":\"Alice\",\"fullName\":\"Alice Maric\"}")
                .get("fullName").asText()).isEqualTo("Alice Maric");

        // A PATCH that does not mention it leaves it alone — the same absent-versus-blank
        // rule the bio has, and the one that used to be got wrong.
        assertThat(patchProfile("{\"displayName\":\"Alice\"}").get("fullName").asText())
                .isEqualTo("Alice Maric");

        // An explicit empty string is how it is removed.
        assertThat(patchProfile("{\"displayName\":\"Alice\",\"fullName\":\"\"}")
                .get("fullName").isNull()).isTrue();
    }

    @Test
    void anOverlongRealNameIsRefusedAndAttachedToItsField() throws Exception {
        String body = mvc.perform(patch("/api/users/{id}", alice.getId())
                        .header(HttpHeaders.AUTHORIZATION, bearer(alice))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"displayName\":\"Alice\",\"fullName\":\"" + "x".repeat(101) + "\"}"))
                .andExpect(status().isBadRequest())
                .andReturn().getResponse().getContentAsString();

        assertThat(body).contains("\"fullName\"");
    }

    private JsonNode patchProfile(String body) throws Exception {
        return okJson(patch("/api/users/{id}", alice.getId())
                .header(HttpHeaders.AUTHORIZATION, bearer(alice))
                .contentType(MediaType.APPLICATION_JSON)
                .content(body));
    }
    ...
    void noEndpointThatReturnsAUserEverReturnsAnEmailOrACredential() throws Exception {
        ...
        String[] bodies = {
            ...
            okBody(get("/api/users/search").param("q", "alice")),
