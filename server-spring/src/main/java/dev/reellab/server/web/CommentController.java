...
public class CommentController {
    ...
    // A comment carries its author, and an author carries an avatar — a relative path in
    // storage, an absolute URL in a response. This is what does that one composition.
    private final MediaUrlAssembler media;

    public CommentController(CommentService comments, MediaUrlAssembler media) {
    ...
    private CommentResponse toResponse(CommentEntity comment, List<CommentEntity> replies) {
        return new CommentResponse(
                comment.getId(),
                UserResponse.from(comment.getAuthor(), media.toUrl(comment.getAuthor().getAvatarPath())),
