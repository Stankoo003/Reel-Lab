...
public class GlobalExceptionHandler {
     ...
     *
     * <p>429 rather than a silent success. This one CAN be honest about what happened without
     * leaking anything: it is a fact about the caller's own behaviour, and it fires whether or
     * not the address has an account — so it tells an enumerator nothing it did not already
     * know about itself.
     */
    /**
     * Blocked. 403 rather than 404 or a silent success: the sender has to know the message
     * did not arrive, or they will keep typing into a void. The message deliberately does not
     * say who blocked whom — see BlockService.BLOCKED.
     */
    @ExceptionHandler(BlockService.BlockedException.class)
    public ProblemDetail onBlocked(BlockService.BlockedException ex) {
        return ProblemDetail.forStatusAndDetail(HttpStatus.FORBIDDEN, ex.getMessage());
    }

    @ExceptionHandler(PasswordResetService.TooManyRequestsException.class)
    public ProblemDetail onRateLimited(PasswordResetService.TooManyRequestsException ex) {
