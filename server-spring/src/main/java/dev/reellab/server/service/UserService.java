...
public class UserService {
    ...
    /** How many results one search returns when the caller does not say. */
    public static final int SEARCH_LIMIT = 25;

    /** The ceiling on what a caller may ask for, so a query string cannot page the table out. */
    public static final int SEARCH_LIMIT_MAX = 50;

    /**
     * Find people by handle or display name.
     *
     * <p>An empty query returns nothing rather than everyone. A search box with nothing typed
     * in it is not a request for the whole user table, and answering one that way turns the
     * first keystroke into the most expensive query the screen ever makes.
     *
     * <p>A leading {@code @} is dropped, because that is how the app writes handles everywhere
     * else and pasting one in should find the person, not nobody.
     */
    @Transactional(readOnly = true)
    public List<UserEntity> search(String query, Integer limit) {
        ...
        /*
         * LIKE gives % and _ meaning, so a user who types either would otherwise be running a
         * wildcard search they did not write: "%" alone would match every account on the
         * server. Escaping them — and the escape character itself, first, or the escapes we
         * add would be escaped in turn — makes what was typed match itself literally.
         */
        String literal = typed.toLowerCase(Locale.ROOT)
                .replace("!", "!!")
                .replace("%", "!%")
                .replace("_", "!_");

        int size = Math.clamp(limit == null ? SEARCH_LIMIT : limit, 1, SEARCH_LIMIT_MAX);
        return users.search("%" + literal + "%", literal + "%", PageRequest.of(0, size));
    }
     ...
     */
    @Transactional
    public UserEntity updateProfile(UUID id, String displayName, String fullName, String bio,
                                    String avatarPath) {
        UserEntity user = require(id);
        user.setDisplayName(displayName);
        // Same absent-versus-blank rule as bio below: a field the client did not send is one
        // it has no opinion about, and an empty string is how a name gets removed.
        if (fullName != null) {
            user.setFullName(fullName.isBlank() ? null : fullName.strip());
        }
