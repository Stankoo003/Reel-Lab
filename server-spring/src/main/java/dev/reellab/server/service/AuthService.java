...
public class AuthService {
    ...
    public static final String WRONG_CURRENT_PASSWORD = "That is not your current password.";
    public static final String SAME_PASSWORD = "Choose a password you have not used just now.";
    ...
    /**
     * Change a signed-in account's password.
     *
     * <p>The current password is required even though the caller already holds a valid
     * token: a phone left unlocked is a valid token, and this is the one action where that
     * must not be enough. A wrong current password is a bad credential, answered the way a
     * failed sign-in is, and the new one is held to the same rule as at sign-up.
     *
     * <p>Changing it retires every token minted before — see
     * {@code PasswordChangeTokenValidator} — including the one that made this call. The
     * caller is handed a fresh one so this device stays signed in and every other is out.
     */
    @Transactional
    public UserEntity changePassword(UUID userId, String currentPassword, String newPassword) {
        ...
        user.changePassword(encoder.encode(newPassword), Instant.now());
        return user;
    }
