...
public class VideoService {
    ...
    /**
     * A clip that may be shared into a conversation: one that exists and is published.
     *
     * <p>Empty for a draft as much as for an unknown id, and deliberately so: a draft is
     * visible to its owner alone, and sharing it would be a way around that. The caller
     * cannot tell the two apart, which is the point — an id is a claim, not a key.
     */
    @Transactional(readOnly = true)
    public Optional<VideoEntity> findShareable(UUID id) {
        return videos.findWithOwnerById(id).filter(VideoEntity::isPublished);
    }
